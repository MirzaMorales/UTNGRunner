...
object GameEngine {
    ...
    private fun updateCoins(coins: List<Coin>, speed: Float): List<Coin> {
        ...
        // Spawn logic: spawn a coin probabilistically
        return if (Math.random() < 0.015) {
            val spawnY = if (Math.random() < 0.5) 100f else 140f
            moved + Coin(x = 240f, y = spawnY)
        } else moved
    }
